...
def process_dataset(ds, sender_checkpoint, receiver_checkpoint, hf_token, output_folder, output_file):
    ...
    sender_transcriber = pipeline(task="translation", model=sender_model, tokenizer=sender_tokenizer, device=0)
    receiver_transcriber = pipeline(task="translation", model=receiver_model, tokenizer=receiver_tokenizer, device=0)
    ...
    for sender_out in sender_transcriber(ds_it):
